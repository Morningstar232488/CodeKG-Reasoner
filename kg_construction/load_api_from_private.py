...
# requirement_apis_file = f"../kg_data/requirement_APIs/requirement_libs_private/pytorch-lightning_APIs.json"
# requirement_apis_file = f"../kg_data/parsed_data/err_APIs/err_click_APIs.json"
requirement_apis_file = f"../kg_data/missing_APIs/missing_streamlit_apis.json"
res_file_dir = f"../kg_data/parsed_data/from_private"
    ...
    for item in libs:
        ...
        for entry in item[1].items():
            ...
            if install_single_library(lib, version):
                if "names" in entry[1].keys():
                    details = entry[1]["names"]
                else:
                    details = entry[1]
                for name in details:
                    try:
                        ...
                        # import ipdb
                        # ipdb.set_trace()
                        _, api_path = split_package_and_api(module_path)

                        parsed_info = find_and_parse_api(lib, api_path)
